...
const ModalManagement = ({ children, id, type, title, className,  }) => {
  return (
    <dialog
      id={id}
      className={`modal ${
        type === "large" ? "max-w-none w-4/5 mx-auto" : type === "medium" ? "max-w-none w-4/5 sm:w-3/5 mx-auto" : "mx-auto"
      } `}
    >
      <div
        className={`modal-box scrollbar-hide ${className} bg-white p-[26px] ${
          type === "large" ? "max-w-none w-full" : type === "medium" ? "max-w-none w-full" : "max-w-max  mx-auto"
        }`}
      >
        <div className="mt-0 flex justify-between w-full scrollbar-hide">
          <h3 className=" text-[18px] font-semibold capitalize text-black">{title}</h3>
          <button
